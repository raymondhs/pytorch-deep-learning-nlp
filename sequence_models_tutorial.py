...
def prepare_sentence(sentence, word_to_ix, char_to_ix):
    sentence_in = prepare_sequence(sentence, word_to_ix)
    chars_in = []
    for word in sentence:
        chars_in.append(prepare_sequence(word, char_to_ix))
    return sentence_in, chars_in
...
class CharLSTMTagger(nn.Module):
    ...
    def forward(self, sentence, sentence_chars):
        embeds = self.word_embeddings(sentence) # (seq_length, embedding_dim)
        char_hiddens = []
        for word in sentence_chars:
            ...
            char_lstm_out, self.char_hidden = self.char_lstm(
                char_embeds.view(len(word), 1, -1), self.char_hidden)
            char_hiddens.append(char_lstm_out[-1])
        # (seq_length, 1, char_embedding_dim)
        char_hiddens = torch.stack(char_hiddens)
        # (seq_length, 1, embedding_dim+char_embedding_dim)
        embeds = torch.cat((embeds.view(len(sentence), 1, -1), char_hiddens), dim=2)
...
model = CharLSTMTagger(EMBEDDING_DIM, HIDDEN_DIM, len(word_to_ix), len(tag_to_ix),
                      CHAR_EMBEDDING_DIM, CHAR_HIDDEN_DIM, len(char_to_ix))
loss_function = nn.NLLLoss()
optimizer = optim.SGD(model.parameters(), lr=0.1)
...
with torch.no_grad():
    ...
    inputs, chars = prepare_sentence(sentence, word_to_ix, char_to_ix)
    tag_scores = model(inputs, chars)
    print(tag_scores)
...
for epoch in range(300):  # again, normally you would NOT do 300 epochs, it is toy data
    for sentence, tags in training_data:
        ...
        # Step 2. Get our inputs ready for the network, that is, turn them into
        # Tensors of word indices.
        sentence_in, chars_in = prepare_sentence(sentence, word_to_ix, char_to_ix)
        targets = prepare_sequence(tags, tag_to_ix)
